...
async function fetchAgentPages() {
	...
	return data.query.categorymembers.filter(
		(page) => !['Agent', 'Agent/List'].includes(page.title),
	);
}
...
async function fetchRosterAttributes() {
	...
	$('table').each((i, table) => {
		...
		if (!headerText.includes('Attribute') || !headerText.includes('Specialty'))
			return;
...
async function main() {
	...
	console.log(
		`Parsed attributes for ${rosterAttributes.size} agents from roster table.`,
	);

	// TEMP DEBUG: confirm the first agent's attributes look right before trusting the rest
	if (agents[0]) {
		console.log(
			`Sample attributes for "${agents[0].title}":`,
			rosterAttributes.get(agents[0].title),
		);
	}
	...
	for (const agent of agents) {
		...
		if (['Belle', 'Wise'].includes(agent.title)) continue; // non-combat protagonists, no Attribute/Specialty
		...
		try {
			...
			const attrs = rosterAttributes.get(agent.title) || {
				element: null,
				specialty: null,
				faction: null,
			};
	...
	const missingAttrs = manifest.filter(
		(m) => !m.element || !m.specialty || !m.faction,
	);
	if (missingAttrs.length > 0) {
		console.log(
			`\n${missingAttrs.length} agent(s) missing attribute data (likely a name mismatch with the roster table):`,
		);
		missingAttrs.forEach((m) => console.log(' -', m.name));
	}
